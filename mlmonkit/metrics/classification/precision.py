...
class Precision(BaseMetric):
    """
    Class to calculate precision for both batch and streaming evaluation modes.
    
    Attributes:
    use_streaming (bool): If True, streaming evaluation is enabled; otherwise, 
                          batch evaluation is used.
    true_positive (int): The number of true positive predictions seen so far 
    (used in streaming mode).
    total_predicted_positive (int): The total number of predicted positive cases 
    seen so far (used in streaming mode).
    """

    def __init__(self, name='Precision', use_streaming=False):
        """
        ...
        if self.use_streaming:
            ...
            self.total_predicted_positive = 0  # Total predicted positives

    def evaluate_batch(self, true_labels, pred_labels):
        """
        Evaluates precision in batch mode by comparing true and predicted labels.
        ...
        Returns:
        float: Precision score, calculated as true positives divided by total predicted positives.
        ...
        total_predicted_positive = np.sum(pred_labels == 1)

        return true_positive / total_predicted_positive if total_predicted_positive > 0 else 0.0

    def evaluate_streaming(self, true_labels, pred_labels):
        """
        Evaluates precision incrementally in streaming mode.
        ...
        Returns:
        float: Updated precision score, calculated incrementally.
        ...
        self.total_predicted_positive += np.sum(pred_labels == 1)

        return self.true_positive / self.total_predicted_positive \
            if self.total_predicted_positive > 0 else 0.0

    def do_score(self, true_labels, pred_labels):
        """
        Updates the precision calculation based on the selected mode (batch or streaming).
        ...
        Returns:
        float: Precision score based on the mode selected (batch or streaming).
